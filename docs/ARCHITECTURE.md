...
- **CP2** — Sound integration (`2e478b1`). `src/lib/sound.ts` with five named functions (win / lose / draw / earn / dailyUnlock) gated on `useSettingsStore.sound`. `createAudioPlayer` is lazy — players are initialized on first `play()` call via the `getPlayer()` helper (`sound.ts:74`), not at module load. Module-load work is limited to resolving asset numbers into the `SOURCES` map; the `players` record is initialized to all-nulls and populated on demand. Mitigates startup memory cost; first-call latency is negligible in practice because `createAudioPlayer` returns synchronously and starts loading in the background. `seekTo(0) → play()` semantics restart on rapid repeat. Five procedurally generated WAVs (ffmpeg sine + amix + afade) at `assets/sounds/` as placeholders for production-licensed assets (queued in Phase 9). Global jest mock at the `@/lib/sound` level + native-binding mock at `expo-audio`.
...
Phase 7A.7 is programmatic-green at `1514/1514` tests, manual-sanity verified end-to-end on iOS Simulator across the six Mode 2-7 per-mode tutorials, the HomeScreen interception flow (first tap → tutorial → Matchmaking; re-tap → direct Matchmaking), and Mode 7's race ecosystem polish (race-aware result copy, animated badge, tighter bot pace, mid-match haptic). Mode 1's tutorial is owned by Phase 7A.6's `TutorialMatchScreen` during the linear onboarding flow — it is NOT a per-mode tutorial and Mode 1's ModeCard tap is explicitly exempt from the CP7 interception (`HomeScreen.tsx:playMode` short-circuits on `entry.id === 1`; `ModeTutorialScreen`'s `slidesForMode(1)` returns null as a defensive fallback).

**Phase 7A.8 inherits**:
- Modes 2-7 per-mode tutorials live and reachable through the production ModeCard tap interception; Mode 1's tutorial remains in Phase 7A.6's `TutorialMatchScreen`
